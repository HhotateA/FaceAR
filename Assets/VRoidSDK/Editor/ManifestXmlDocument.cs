...
namespace VRoidSDK.Editor
{
    public class ManifestXmlDocument
    {
        ...
        {
            List<XmlNode> categoryNodes = FindChildrenNodeByName(ManifestDocument, "category");
            ...
            foreach (IGrouping<XmlNode, XmlNode> parentCategories in nodeCategories)
            {
                if (HasDefaultBrowsableCategory(parentCategories))
                {
                    ...
                    dataNode = ManifestDocument.CreateElement("data");
        ...
        private XmlNode FindUri(List<XmlNode> parentCatrgoryNodes, Uri uri)
        {
            List<XmlNode> dataNodes = parentCatrgoryNodes.FindAll((node) => node.Name == "data");
